    ...
    scanApplication(app, options) {
        ...
        const denormalizedPaths = modules.map(({ controllers, metatype, imports }) => {
            ...
            if (options.apiVersion) {
                result = result.filter((moduleRoute) => Array.isArray(moduleRoute.root.version)
                    ? moduleRoute.root.version.includes(options.apiVersion) ||
                        moduleRoute.root.version.includes(interfaces_1.VERSION_NEUTRAL)
                    : moduleRoute.root.version === options.apiVersion ||
                        moduleRoute.root.version === interfaces_1.VERSION_NEUTRAL);
            }
