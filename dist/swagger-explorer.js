    ...
    exploreRoutePathAndMethod(instance, prototype, method, metatype, globalPrefix, modulePath, applicationConfig) {
        ...
        return (0, lodash_1.flatten)(allRoutePaths.map((routePath) => {
            ...
            if (requestMethod === common_1.RequestMethod.ALL) {
                ...
                return validMethods.map((meth) => (Object.assign({ method: meth.toLowerCase(), path: fullPath === '' ? '/' : fullPath, operationId: `${this.getOperationId(instance, method)}_${meth.toLowerCase()}`, version: versionOrVersions }, apiExtension)));
            }
            const pathVersion = versions.find((v) => fullPath.includes(`/${v}/`));
            return Object.assign({ method: common_1.RequestMethod[requestMethod].toLowerCase(), path: fullPath === '' ? '/' : fullPath, operationId: this.getOperationId(instance, method, pathVersion), version: versionOrVersions }, apiExtension);
        }));
    }
